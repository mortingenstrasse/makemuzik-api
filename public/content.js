function loadProductById(productId) {
  fetch(`https://www.makemuzik.com/api/products/${productId}?populate=*`)
    ...
    .then(data => {
      ...
      if (Image && Image.data && Array.isArray(Image.data) && Image.data.length > 0) {
        ...
        if (imageFormats.medium) {
          imageUrl = `https://www.makemuzik.com${imageFormats.medium.url}`;
        } else if (imageFormats.small) {
          imageUrl = `https://www.makemuzik.com${imageFormats.small.url}`;
        } else if (imageFormats.thumbnail) {
          imageUrl = `https://www.makemuzik.com${imageFormats.thumbnail.url}`;
        } else {
          imageUrl = `https://www.makemuzik.com${Image.data[0].attributes.url}`;
        }
